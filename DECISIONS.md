...
| 2026-01-26 | Tool types stay in `src/inference/`, not `src/tools/` | Semantic separation: `inference/` = API data structures (ToolCall, ToolDefinition), `tools/` = ergonomic Rust abstractions (Tool trait, ToolRegistry). Consistent with other API types (ChatRequest, GenerateRequest). Feature flag gates correctly without relocation. | DECISIONS.md |
